...
5. **~~Works at the tank, offline.~~ Superseded 2026-09-10 — accounts are now required.** Jaideep: "I want to launch it to real users... a backend to store everything." Real user accounts (Google sign-in, or phone number + a self-chosen 4-digit PIN) now exist, and every route requires sign-in (`src/proxy.ts`). Core data — tanks, livestock, measurements, profile (2026-09-10), plus plants, equipment, journal/log entries, and photos (2026-09-11) — is stored server-side in Postgres, scoped per account, specifically so it follows a user across devices; photo files themselves live on Vercel Blob (public store — unguessable URL, not access-controlled), not local browser storage. The remaining local-only tables (scans, AI interactions, dexCards, dismissedWarnings, settings, speciesSuggestions, the species catalog, parameterDefs) are mid-migration to the same pattern; until that's done, those specific screens still depend on local SQLite-WASM/OPFS and won't yet follow a user to a new device. See `specs/PROGRESS.md`'s 2026-09-10 and 2026-09-11 "accounts + backend" entries for the full decision record and what's still local vs. server-backed as of any given date.
...
- **SQLite (via WASM) is still the source of truth for tables not yet migrated server-side** (see Principle 5's note) — for `tanks`/`livestock`/`livestockEvents`/`measurements`/`profile`/`plants`/`equipment`/`logEntries`/`photos`, Postgres is now the source of truth instead, and photo bytes live on Vercel Blob, not OPFS (`src/lib/photo-upload.ts`'s `uploadPhoto()`, not `src/lib/opfs-files.ts`'s `writePhotoFile()`, for any of those). Zustand is UI state. TanStack Query wraps network calls only. Do not cache local SQLite data in TanStack Query — Drizzle live queries already give reactive re-rendering, and layering a cache creates two sources of truth that drift.
