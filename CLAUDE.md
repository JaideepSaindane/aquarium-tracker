...
4. **Their data is theirs.** Full CSV + JSON + photo export on the free tier, tied to their account. This ships early (T-012), not "later." (The "no account required" half of this principle was retired 2026-09-10 — see Principle 5's note.) **Narrowed 2026-09-15 for Jaideep's own admin access**: the admin dashboard (`/admin`, T-030) originally showed aggregate, account-agnostic numbers only, by explicit design, specifically so it never exposed an individual user's own tank/fish/journal/AI-chat content. Jaideep asked for that narrowed: `/admin/accounts` and `/admin/accounts/[id]` now show real per-account detail, including reading the actual text of a user's Ask AquaAI questions and answers (`ai_interactions.userInput`/`.response`, kind `"ask"`). This is a deliberate, acknowledged exception — Jaideep, the product owner, can see what any account asked the AI — not a general erosion of user privacy from other staff or third parties; there is still no admin-role system beyond the email allowlist in `src/server/auth/require-admin.ts`. If the app ever states elsewhere (a privacy policy, marketing copy) that conversations are private to the user, that copy needs to reflect this exception too — do not assume it already does.
5. **~~Works at the tank, offline.~~ Superseded 2026-09-10 — accounts are now required.** Jaideep: "I want to launch it to real users... a backend to store everything." Real user accounts (Google sign-in, or phone number + a self-chosen 4-digit PIN) now exist, and every route requires sign-in (`src/proxy.ts`). All real per-user data — tanks, livestock, measurements, profile, plants, equipment, journal/log entries, photos, Tank Scan/Health Check history, Ask AquaAI history, Species Dex unlocks, dismissed warnings, account settings, species suggestions, and per-tank parameter overrides — is stored server-side in Postgres, scoped per account, specifically so it follows a user across devices; photo files themselves live on Vercel Blob (public store — unguessable URL, not access-controlled), not local browser storage (`src/lib/photo-upload.ts`'s `uploadPhoto()`, never `src/lib/opfs-files.ts`'s `writePhotoFile()`, for any of the above). The species catalog (1,484 curated species) is the one deliberate exception, staying local by design, not oversight — it's shared reference data identical for every user, not per-user data, so it doesn't belong in a per-account table; it's still re-seeded locally from `data/species.seed.json` as before. See `specs/PROGRESS.md`'s 2026-09-10 and 2026-09-11 "accounts + backend" entries for the full decision record.
...
| Local database | SQLite compiled to WASM (`wa-sqlite` or `sql.js`) + Drizzle ORM, persisted via OPFS — now only used for the shared species catalog (see Principle 5's note); every other table is server-side |
...
- **Postgres is the source of truth for every real per-user table now** (see Principle 5's note) — SQLite/OPFS is only still used for the shared species catalog. Photo bytes live on Vercel Blob, not OPFS. Zustand is UI state. TanStack Query wraps network calls only. Do not cache local SQLite data in TanStack Query — Drizzle live queries already give reactive re-rendering, and layering a cache creates two sources of truth that drift.
...
- **Never put an AI API key in client-side code or a bundle.** All model calls go through Next.js API routes (server-side) acting as the proxy, always using the server's own key. Bring-your-own-key mode (T-013/T-025) was removed 2026-09-12: it was fully wired end-to-end server-side but never had an actual UI for a user to enter a key, so it was dead code nobody could reach, not a real feature — see `specs/PROGRESS.md`'s 2026-09-12 entry. Do not rebuild it without Jaideep asking again.
...
- **Medication/dosing/disease questions may now be answered from general knowledge when no vet-reviewed corpus entry covers them — reversed 2026-09-11/12 at Jaideep's explicit request.** Until this change, if retrieval returned nothing for a question about treating a sick/injured fish, the answer was a hard refusal ("I don't have grounded guidance on this yet"), never a guess. Jaideep asked for that removed so the app gives a real answer with a disclaimer instead, while the corpus's Tier 1 (medication) entries still wait on real vet review. The app now sets `medical_disclaimer: true` on any such ungrounded answer (`ask/v4`, `triage/v2`) and the UI shows a loud, distinct "not yet vet-reviewed, confirm with a vet before trying" warning — separate from the softer `uncovered` badge used for ordinary general-knowledge care facts. **This is a real, acknowledged safety tradeoff, not a refinement**: an ungrounded medication answer is the same shape as an uncredentialed forum post, just with a warning label attached; nothing stops a keeper from acting on it anyway. Do not extend this further (e.g. quietly dropping the disclaimer, or applying it to a case Jaideep didn't ask about) without him asking again. (A plain species care fact like a temperature or pH range was already narrowed out of the medication/dosing/disease bucket on 2026-09-11 — see the note below — that narrowing stands independent of this later change.)
...
- **Sourced-but-unreviewed corpus entries are retrievable, and always carry the disclaimer (2026-09-16, Jaideep's call).** 30 disease/medication entries (ich, columnaris, velvet, flukes, salt, copper, formalin, dosing rules…) were written and sourced but held at `status: sourced` awaiting vet review, which made them invisible to retrieval — so disease answers fell back to the model's own general knowledge instead, which is worse. `retrieveCorpus()` now returns `live` **and** `sourced` entries; a `sourced` one is prefixed with an `[UNREVIEWED ENTRY …]` banner and both prompts (`ask.v5`, `triage.v3`) are told that anything taken from it counts as not vet-reviewed, so `medical_disclaimer` stays true and the loud amber warning always shows. Nothing below `sourced` (draft/stale) is ever eligible. Do not "clean this up" by promoting those entries to `live` — `live` still means a named human reviewed it (`docs/05-content-guide.md`), and promoting them would silently drop the only warning a user gets.
