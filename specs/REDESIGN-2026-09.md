...
**Status:** Sections 0–8 complete (2026-09-12/13). Next: Section 9 (Profile + Settings). Work the sections in order, one at a time — Jaideep says "do Section N".
...
**Done:** `src/app/tank/[id]/page.tsx` rebuilt per the brief's mockup. Header is now name + water-type badge followed by a real `Status` (Section 2 component — icon + label + a required explanation, never a bare pill): no fish → neutral "No fish yet / Add your first fish to get started"; any open equipment warning (from the existing `checkFilterFlow`/`checkHeaterWattage` derived checks) → watch "Needs attention / {n} equipment check(s) need a look"; otherwise → improve "Healthy / {n} fish · nothing flagged". This replaces the old hardcoded "● Healthy" text that had no backing data at all. `AboutSection` reworked so Volume and Temperature are the two primary metrics (large, side-by-side) with `{L}×{W}×{H} cm · Created {date} · Planted` demoted to a small fine-print line underneath — dimensions and creation date no longer compete with the numbers that actually answer "how is this tank doing." A new Water section (in a `Card`, wrapped in `<Link href="/tank/[id]/log">`) lists pH/Ammonia/Nitrite/Nitrate with the latest logged value and a ✓/⚠ mark against each parameter's target range (`getEffectiveParameterDefs` + `getLastMeasurement`, in-range logic ported from the same shape as `checkParameterOutOfRange`) — **hidden entirely, not shown with placeholder rows, until at least one parameter has actually been logged** (see decisions-log entry below). This incidentally fixes a real pre-existing bug: nothing in the app previously linked to `/tank/[id]/log` at all; it was only reachable by typing the URL. New i18n keys added in both `en.ts` and `hi-latn.ts`: `home.statusNeedsAttention(Explanation/Plural)`, `tankOverviewPage.water`.
**Reverted same day, per decisions-log entry below**: the original build also replaced Fish/Gallery/Journal's three heavy inline accordions (`CollapsedSectionCard`, `StackedThumbs`, `LivestockInlineRow`, plus their own add-fish popup, `GalleryPanel`, `JournalPanel`) with three simple `ListRow`s navigating to the dedicated pages. Jaideep asked to keep the original accordions ("I liked them the way they were before") — they're restored verbatim from the pre-Section-4 commit, unchanged. Only the header/Status, AboutSection, and Water card are Section 4's real, standing changes.
**Verification:** `npx tsc --noEmit` and `npm run build` clean; `npm run lint` unchanged (only the 4 pre-existing `react-hooks/refs` errors in `tank/[id]/livestock/page.tsx`, untouched by this change). Real-browser pass on a local dev server with a throwaway phone+PIN account: confirmed the zero-fish state (honest "No fish yet" status, "36 L / —" metrics with "Add fish for a range," Water card correctly absent with nothing logged, Fish/Gallery/Journal accordions expanding inline exactly as before); a separate earlier pass (before the accordion revert) added a Betta and confirmed status flipped to "Healthy · 1 fish · nothing flagged," logged Ammonia = 2 (above the 0.25 ppm target) and pH = 7 (within 6.5–7.5), and confirmed the Water card showed **pH 7.0 ✓** and **Ammonia 2.00 ⚠**. **Cleaned up after each verifying pass**: same shared production Postgres as prior sections — deleted the throwaway test accounts via the real self-serve "Delete my account" flow.
**Acceptance:** Met — the screen now leads with an honest, derived health status and real water-parameter data (shown only once it exists) instead of a hardcoded badge and empty placeholder rows; Fish/Gallery/Journal stay as Jaideep's preferred inline accordions.

## Section 5 — Onboarding + tank creation + units (brief Phase 3) — COMPLETE (2026-09-13)

**Goal:** A new user reaches a real tank with minimal friction.
**Done:** Onboarding (`src/app/onboarding/page.tsx`) stripped down to Welcome → name only — city and the language/theme toggles are gone (both already live in Settings, per the brief's own Screen 4 note: "the current onboarding puts too many unrelated controls in one place"). Get Started now saves the name and routes straight into `/onboarding/scan` ("Show us your aquarium" — the existing photo-first scan flow, retitled from "Scan your tank" per the brief's own wording) instead of dropping the user on an empty My Tanks list; a "Skip for now" link bails anyone who doesn't want to do that right now straight to the app (Principle 1: advise, never block). The scan screen's idle stage gained a third option, "Set up manually instead" (`TextButton`), linking to the plain Add Tank form — so a camera-shy or camera-less user still reaches a real tank instead of being stuck choosing between two AI-photo paths.

New app-wide units system: `src/lib/units.ts` (pure formatters: `formatLength`/`formatVolume`/`formatTemp`/`formatTempRange`, plus `formatVolumeDual` for the brief's own always-both-units example) + `src/lib/UnitsProvider.tsx`, a `localStorage`-backed preference (same reasoning as `ThemeProvider`/`LocaleProvider` — a per-device display choice, not per-user data, so it stays outside the Postgres `settings` table) exposing `useUnits()`. Settings gained a "Units" `SegmentedControl` (Metric/Imperial) next to Appearance. Tank Detail's `AboutSection` (Volume/Temp primary metrics, the dimensions fine-print line) now reads through `useUnits()` — switching the Settings preference changes Tank Detail's numbers immediately, in both directions (cm↔in, L↔gal, °C↔°F). The three dimension-entry screens with their own cm/ft input toggle (`tank/new`, `tank/[id]/size`, `onboarding/scan`) keep that toggle as-is (a data-entry convenience, not the display-layer preference) but their live "≈ X L" volume preview now always shows the brief's own dual format, `formatVolumeDual` (`54.9 L · 14.5 gal`), instead of a single metric-only number.

Add Tank (`tank/new`) rebuilt per the brief: the cm/ft toggle now uses the real `SegmentedControl` primitive instead of two hand-rolled buttons; the water-type picker is now two real `ChoiceCard`s (icon + title + subtitle, aqua border + checkmark when selected) instead of two custom-styled buttons; City and the age-band field ("How old is your tank?") moved into a collapsed native `<details>` "More details" disclosure, since name/dimensions/water-type are the only things standing between a user and a real, usable tank — everything else can be filled in later.

Planner (`onboarding/planner`) steps 1–3 (the pre-plan wizard, not the results screen) gained a `StepIndicator` ("Step N of 3") under each `BackHeader`, and step 1's tank-type buttons plus step 3's shape buttons are now real `ChoiceCard`s. Steps 1 and 2's "Next — ..." footer buttons were simplified to a plain `t.common.continue` ("Continue") per the brief's own ask; step 3's "Build my plan" stays as-is since it names a real action, not a step transition. Step 2's existing "Not sure yet? These are proven first fish" chip row already satisfied the brief's "Not sure?" beginner-help ask and was left as-is.
**Decisions-log entry**: this supersedes the 2026-09-04/05 profile-first onboarding (name+city+language+theme, landing on My Tanks) — see the matching `specs/PROGRESS.md` entry.
**Verification:** `npx tsc --noEmit`, `npm run build`, and `npm run lint` all clean (lint unchanged: same 4 pre-existing `react-hooks/refs` errors in `tank/[id]/livestock/page.tsx`). Real-browser pass on a local dev server with a throwaway phone+PIN account: confirmed the simplified Welcome screen (name field, Get Started, Skip for now, no city/language/theme), confirmed Get Started lands on the retitled "Show us your aquarium" scan screen with the new "Set up manually instead" link working (routes to Add Tank); on Add Tank confirmed the `SegmentedControl` cm/ft toggle, the `≈ 54 L · 14.3 gal` dual-volume line, the two `ChoiceCard` water-type options, and the collapsed "More details" correctly holding City/age-band; saved a real tank and confirmed Tank Detail showed `54 L` / `60 cm × 30 cm × 30 cm` in Metric; flipped Settings to Imperial and confirmed the same tank now reads `14.3 gal` / `24 in × 12 in × 12 in` with no page-specific toggle needed; walked all three Planner pre-plan steps and confirmed the "Step N of 3" indicator, the `ChoiceCard` tank-type/shape pickers, and the plain "Continue" CTA. **Cleaned up after verifying**: same shared production Postgres as prior sections — deleted the throwaway test account via the real self-serve delete flow.
**Acceptance:** Met — a new user reaches a real tank in two taps (name → Get Started → photo or manual setup), and the Settings units preference changes Tank Detail's numbers app-wide; per-parameter units (Water card ppm/pH/°C readings, log-entry input units) are unchanged and out of scope for this pass — flagging for a later section if Jaideep wants that too.
