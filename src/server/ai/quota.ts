...
function currentMonthKey(deviceId: string, kind: "scan" | "ask"): string {
  const now = new Date();
  const month = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}`;
  return `quota:${deviceId}:${kind}:${month}`;
}
...
/** Checks quota WITHOUT incrementing — use before doing the (expensive) AI call. */
export async function peekQuota(deviceId: string, kind: "scan" | "ask"): Promise<QuotaCheck> {
  const redis = getRedis();
  const key = currentMonthKey(deviceId, kind);
...
/** Call only after the AI call actually succeeds — a failed call shouldn't burn quota. */
export async function incrementQuota(deviceId: string, kind: "scan" | "ask"): Promise<void> {
  const redis = getRedis();
  const key = currentMonthKey(deviceId, kind);
