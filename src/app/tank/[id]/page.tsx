...
export default function TankOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Screen>
        ...
        <Card>
          ...
          {(waterRows ?? []).map((row) => (
            <div key={row.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0" }}>
              ...
              {row.value == null ? (
                <span style={{ fontSize: "var(--font-caption-size)", color: "var(--color-ink-faint)" }}>{t.tankOverviewPage.notLogged}</span>
              ) : (
      ...
      {/*
       * Redesign brief Screen 2, High-priority finding: "Fish/Gallery/
       * Journal accordions are visually heavy... the user doesn't need
       * three giant bordered accordions." These now navigate to their
       * existing, already-full-featured dedicated pages (Fish's own add
       * flow — search or photo — lives on /tank/[id]/livestock, same for
       * Gallery/Journal) instead of duplicating an inline expand-in-place
       * accordion, add popup, and panel on this screen too. Nothing is
       * lost: those pages already have their own add affordances; this
       * removes only the duplication, not the capability.
       */}
      <Card style={{ marginBottom: 16 }}>
        <ListRow
          icon="🐟"
          label={t.livestockPage.fish}
          meta={aliveLivestock.length > 0 ? t.home.fishCount.replace("{n}", String(aliveLivestock.reduce((sum, l) => sum + l.count, 0))) : t.tankOverviewPage.noFishAddedYetShort}
          trailing={`⊕ ${t.common.add}`}
          showChevron
          onClick={() => router.push(`/tank/${id}/livestock`)}
        />
        <ListRow
          icon="📷"
          label={t.galleryPage.title}
          meta={(photos ?? []).length > 0 ? t.tankOverviewPage.nPhotos.replace("{n}", String((photos ?? []).length)) : t.tankOverviewPage.noPhotosYet}
          showChevron
          onClick={() => router.push(`/tank/${id}/gallery`)}
        />
        <ListRow
          icon="📓"
          label={t.journalPage.title}
          meta={(journalEntries ?? []).length > 0 ? t.tankOverviewPage.nEntries.replace("{n}", String((journalEntries ?? []).length)) : t.tankOverviewPage.noEntriesYet}
          showChevron
          onClick={() => router.push(`/tank/${id}/journal`)}
        />
      </Card>
    </Screen>
  );
