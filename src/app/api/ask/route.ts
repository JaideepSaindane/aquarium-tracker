...
export async function POST(req: NextRequest) {
  ...
  // x-device-id header (2026-09-11 fix — the old device-id quota was
  // trivially bypassable by clearing browser storage, since it had no
  // link to the account a user was actually signed into).
  const userId = await requireUserId();
  if (!userId) return NextResponse.json({ error: "unauthorized" }, { status: 401 });

  const quota = await peekQuota(userId, "ask");
  if (!quota.allowed) {
  ...
  const promptText = await loadPrompt("ask.v5.md", {
    QUESTION: question,
  ...
  await incrementQuota(userId, "ask");
